...
## 6.1 Shooter 的语义与「锁定攻击权」

**Shooter 是本轮攻击函数的数学发射锚点**，而不是「必须活到开火瞬间、否则攻击作废的枪手」。

- **击杀对方的 Shooter 不会取消对方本轮的攻击。** START 之后双方的攻击权就已经锁定：
  即使你的 Shooter 在这一轮开火之前被对方击杀，只要你在自己的时间预算内交出了
  合法函数，你的攻击**照样执行**，且仍然必须满足 `f(x_shooter) = y_shooter`——
  用的还是 START 那一份快照里的 Shooter 坐标。
- 攻击**不执行**的唯一原因是算法侧的问题：`TIMEOUT`、`INVALID`、`CRASH`。
  被击杀不是其中之一。
- **本轮不换人。** Shooter 阵亡后不会随机改选另一个存活点，也不会重新运行你的算法；
  本轮输入快照（`public_state.json` / `reveal_state.json`）在 START 后冻结，不因第一击的结果而改变。
- 阵亡的 Shooter **不会**在后续轮次被重新选中（它已经 `alive: false`），
  但下一轮选谁仍然由人类操作员按正常流程决定，不会自动指定。

**那还有必要打对方 Shooter 吗？有。** 击杀它是一个**正常的击杀**：它从对方场上移除一个点，
并因此影响后续轮次的可选 Shooter 与存活数。要不要瞄准它，是一个真实的策略取舍，而不是「一击必杀」的开关。

> 规则修订说明：早期 V1.1 试玩规则曾规定「先手击杀对方 Shooter ⇒ 对方本轮攻击取消」。
> 该规则经实测显示造成严重的策略坍缩（速度成为胜负的唯一通道），已由人类决定废止。
> 现行规则以本节为准，标记为 **V1.1 Playtest Rules — Revision 2**。
