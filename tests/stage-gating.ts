 ...
 *   3. 阶段推进与真实人工操作一一对应，且 REVEAL 在 START_ROUND **之前**；
...
/** 走到「双方 LOCK」为止：上传 → preflight → 开始比赛 → 选点 → 锁定 */
async function lockedEngine(matchId: string): Promise<MatchEngine> {
  ...
  for (const team of ['A', 'B'] as const) {
    ...
    assert(engine.selectShooter(team, pick.id).ok, `${team} 选点应成功`);
...
test('stage-gating: 未 START 直接 computeRound() 必须抛错（§14/§15）', async () => {
  ...
  assertEqual(engine.getSnapshot().phase, 'LOCKED', 'PRE-REVEAL 后双方已锁定');
...
test('stage-gating: judgeStartRound() 幂等，且未锁定前不得 START', async () => {
  ...
  // 新一轮回到未锁定：此时 START 必须被拒绝
  if (engine.getWinner() === null) {
    assert(!engine.judgeStartRound().ok, '新一轮未锁定前不得 START');
  }
});
...
test('stage-gating: REVEAL 必须发生在 START_ROUND 之前（§3/§25）', async () => {
  ...
  // 先锁 A 再锁 B 时，第二次锁定直接从 A_LOCKED 进入 WAITING_FOR_JUDGE，
  // 因此 B_LOCKED 不必然出现（它只在 B 先锁时出现）—— 这里只要求锁定路径成立。
  for (const p of ['SHOOTER_SELECTION', 'WAITING_FOR_JUDGE', 'REVEAL', 'START_ROUND', 'COUNTDOWN', 'SEND_ROUND_STATE']) {
    assert(idx(p) >= 0, `状态机必须经过 ${p}，实际: ${phases.join(' → ')}`);
  }
  assert(
    idx('A_LOCKED') >= 0 || idx('B_LOCKED') >= 0,
    `状态机必须记录锁定阶段，实际: ${phases.join(' → ')}`
  );
  assert(idx('WAITING_FOR_JUDGE') < idx('REVEAL'), `揭盲必须在双方锁定之后: ${phases.join(' → ')}`);
...
test('stage-gating: 输入字节在冻结后重新生成仍是同一份（同一轮重入不改变哈希）', async () => {
  ...
  assert(threw, '未双方 LOCK 就揭盲必须抛 MatchEngineError');
});
